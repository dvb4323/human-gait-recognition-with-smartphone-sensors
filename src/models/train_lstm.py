...
from lstm import create_lstm, create_bidirectional_lstm, create_gru, compile_model
...
class LSTMTrainer:
    ...
    def evaluate(self, test_ds, X_test, y_test):
        ...
        # Evaluate
        test_loss, test_acc = self.model.evaluate(test_ds, verbose=2)
        print(f"\nTest Loss: {test_loss:.4f}")
        print(f"Test Accuracy: {test_acc:.4f}")

        # Predictions
        y_pred_probs = self.model.predict(X_test, verbose=2)
    ...
    # 4. Tính toán Accuracy sau khi Voting
        ...
        report = classification_report(y_test, y_pred_voted, target_names=class_names)
        print(report)

        # Save report
        report_dict = classification_report(y_test, y_pred_voted, target_names=class_names, output_dict=True)
        with open(self.results_dir / 'classification_report.json', 'w') as f:
            json.dump(report_dict, f, indent=2)

        macro_f1 = report_dict['macro avg']['f1-score']
        print(f"\nMACRO F1-SCORE: {macro_f1:.4f}")
        print(classification_report(y_test, y_pred_voted, target_names=class_names))
