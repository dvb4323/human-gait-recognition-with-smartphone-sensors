...
class PreprocessingPipeline:
    ...
    def run(self):
        ...
        print("-" * 40)
        # Thêm vào cuối hàm run() hoặc trong _split_data()
        print("\n" + "="*30)
        print("KIỂM TRA SỰ TƯƠNG ĐỒNG VAL - TEST")
        print("="*30)

        for name in ['val', 'test']:
            labels = self.data[f'y_{name}']
            unique, counts = np.unique(labels, return_counts=True)
            dist = (counts / len(labels)) * 100
            print(f"Phân phối lớp tập {name.upper()}: {np.round(dist, 2)}%")
    def _load_data(self):
        """Load raw data."""
...
def main():
    config = {
        ...
        'augmentation_factor': 8  # 8x augmentation
    }
